...
function updateShuffleButtonUI() {
    ...
    const label = isRandom ? t("关闭随机播放") : t("开启随机播放");
    button.title = label;
...
function updatePlayModeUI() {
    ...
    if (dom.playModeBtn) {
        ...
        dom.playModeBtn.title = `${t("播放模式")}: ${playModeTexts[mode] || playModeTexts.list}`;
    }
...
function setPlayMode(mode, { announce = true } = {}) {
    ...
    if (announce) {
        ...
        showNotification(`${t("播放模式")}: ${modeText}`);
...
function updatePlayPauseButton() {
    ...
    dom.playPauseBtn.title = isPlaying ? t("暂停") : t("播放");
    if (document.body) {
...
async function togglePlayPause() {
    if (!state.currentSong) {
        if (state.playlistSongs.length > 0) {
            ...
            showNotification(t("播放列表为空，请先添加歌曲"), "error");
        }
        return;
    }
    ...
    if (!dom.audioPlayer.src) {
        try {
            ...
            showNotification(t("播放失败，请稍后重试"), "error");
        }
        return;
    }
    ...
    if (dom.audioPlayer.paused) {
        ...
        if (playPromise !== undefined) {
            playPromise.catch(error => {
                ...
                showNotification(t("播放失败，请检查网络连接"), "error");
            });
        }
    } else {
...
function updateSourceLabel() {
    ...
    dom.sourceSelectButton.setAttribute("aria-label", `${t("当前音源")}：${option.label}，${t("点击切换音源")}`);
    dom.sourceSelectButton.setAttribute("title", `${t("音源")}：${option.label}`);
}
...
async function selectPlaybackQuality(quality) {
    ...
    if (option) {
        showNotification(`${t("音质已切换为")} ${option.label} (${option.description})`);
    }

    if (state.currentSong) {
        const success = await reloadCurrentSong();
        if (!success) {
            showNotification(t("切换音质失败，请稍后重试"), "error");
        }
    }
}
    ...
    function initializeFavoritesEventHandlers() {
        ...
        const handleFavoriteAction = (event, actionButton) => {
            ...
            if (action === "add") {
                ...
                if (added) {
                    renderPlaylist();
                    showNotification(t("已添加到播放列表"), "success");
                } else {
                    showNotification(t("播放列表已包含该歌曲"), "warning");
                }
            } else if (action === "download") {
                event.preventDefault();
                event.stopPropagation();
                ...
                if (removed) {
                    showNotification(t("已从收藏列表移除"), "success");
                }
            }
        };
...
async function performSearch(isLiveSearch = false) {
    ...
    if (!query) {
        showNotification(t("请输入搜索关键词"), "error");
        return;
    }
    ...
    try {
        ...
        dom.searchBtn.innerHTML = `<span class="loader"></span><span>${t("搜索中...")}</span>`;
        ...
        if (state.searchResults.length === 0) {
            showNotification(t("未找到相关歌曲"), "error");
        }

    } catch (error) {
        console.error("搜索失败:", error);
        showNotification(t("搜索失败，请稍后重试"), "error");
        hideSearchResults();
        debugLog(`搜索失败: ${error.message}`);
    } finally {
        // 恢复搜索按钮状态
        dom.searchBtn.disabled = false;
        dom.searchBtn.innerHTML = `<i class="fas fa-search"></i><span>${t("搜索")}</span>`;
    }
}
...
async function loadMoreResults() {
    ...
    try {
        ...
        loadMoreBtn.innerHTML = `<span class="loader"></span><span>${t("加载中...")}</span>`;
        ...
        if (results.length > 0) {
            ...
            showNotification(t("没有更多结果了"));
            debugLog("没有更多结果");
        }
    } catch (error) {
        console.error("加载更多失败:", error);
        showNotification(t("加载失败，请稍后重试"), "error");
        state.searchPage--; // 回退页码
    } finally {
        if (loadMoreBtn) {
            loadMoreBtn.disabled = false;
            loadMoreBtn.innerHTML = `<i class="fas fa-plus"></i><span>${t("加载更多")}</span>`;
        }
    }
}
...
function createSearchResultItem(song, index) {
    ...
    title.textContent = song.name || t("未知歌曲");
...
function importSelectedSearchResults(target = "playlist") {
    ...
    if (songsToAdd.length === 0) {
        resetSelectedSearchResults();
        showNotification(t("未找到可导入的歌曲"), "warning");
        return;
    }
    ...
    if (target === "favorites") {
        ...
        if (added > 0) {
            saveFavoriteState();
            renderFavorites();
            const duplicateHint = duplicates > 0 ? `，${duplicates} ${t("首已存在")}` : "";
            showNotification(`${t("成功导入")} ${added} ${t("首收藏歌曲")}${duplicateHint}`, "success");
        } else {
            updateFavoriteActionStates();
            showNotification(t("选中的歌曲已在收藏列表中"), "warning");
        }
        updateFavoriteIcons();
        return;
    }
    ...
    if (added > 0) {
        renderPlaylist();
        const duplicateHint = duplicates > 0 ? `，${duplicates} ${t("首已存在")}` : "";
        showNotification(`${t("成功导入")} ${added} ${t("首歌曲")}${duplicateHint}`, "success");
    } else {
        updatePlaylistActionStates();
        showNotification(t("选中的歌曲已在播放列表中"), "warning");
    }
    updateFavoriteIcons();
}
...
function createLoadMoreButton() {
    ...
    button.type = "button";
    button.innerHTML = `<i class="fas fa-plus"></i><span>${t("加载更多")}</span>`;
...
function displaySearchResults(newItems, options = {}) {
    ...
    if (itemsToAppend.length === 0 && state.renderedSearchCount === 0 && totalCount === 0) {
        container.innerHTML = `<div style="text-align: center; color: var(--text-secondary-color); padding: 20px;">${t("未找到相关歌曲")}</div>`;
        state.renderedSearchCount = 0;
...
function showQualityMenu(event, index, type) {
    ...
    menu.innerHTML = `
        <div class="quality-option" onclick="downloadWithQuality(event, ${index}, '${type}', '128')">${t("标准音质")} (128k)</div>
        <div class="quality-option" onclick="downloadWithQuality(event, ${index}, '${type}', '192')">${t("高音质")} (192k)</div>
        <div class="quality-option" onclick="downloadWithQuality(event, ${index}, '${type}', '320')">${t("超高音质")} (320k)</div>
        <div class="quality-option" onclick="downloadWithQuality(event, ${index}, '${type}', '999')">${t("无损音质")}</div>
    `;
...
async function downloadWithQuality(event, index, type, quality) {
    ...
    try {
        ...
        showNotification(t("下载失败，请稍后重试"), "error");
    }
}
...
async function playSearchResult(index) {
    ...
    try {
        ...
        showNotification(`${t("正在播放")}: ${song.name}`);
...
function exportPlaylist() {
    if (!Array.isArray(state.playlistSongs) || state.playlistSongs.length === 0) {
        showNotification(t("播放列表为空，无法导出"), "warning");
        return;
    }
    ...
    try {
        ...
        showNotification(`${t("已导出")} ${state.playlistSongs.length} ${t("首歌曲")}`, "success");
    } catch (error) {
        console.error("导出播放列表失败:", error);
        showNotification(t("导出失败，请稍后重试"), "error");
    }
}
...
function handleImportPlaylistChange(event) {
    ...
    reader.onload = () => {
        try {
            ...
            if (added > 0) {
                ...
                showNotification(t("文件中的歌曲已在播放列表中"), "warning");
            }
        } catch (error) {
            console.error("导入播放列表失败:", error);
            showNotification(t("导入失败，请确认文件格式"), "error");
        } finally {
            if (input) {
                input.value = "";
            }
    ...
    reader.onerror = () => {
        ...
        showNotification(t("无法读取播放列表文件"), "error");
        if (input) {
            input.value = "";
        }
    };
...
function removeFromPlaylist(index) {
    ...
    if (removingCurrent) {
        if (state.playlistSongs.length === 1) {
            ...
            dom.currentSongTitle.textContent = t("选择一首歌曲开始播放");
            updateMobileToolbarTitle();
            dom.currentSongArtist.textContent = t("未知艺术家");
            showAlbumCoverPlaceholder();
            clearLyricsContent();
            if (dom.lyrics) {
    ...
    updatePlaylistActionStates();
    savePlayerState();
    showNotification(t("已从播放列表移除"), "success");
...
function toggleFavorite(song) {
    ...
    if (!key) {
        showNotification(t("无法收藏该歌曲"), "error");
        return;
    }
    ...
    if (existingIndex >= 0) {
        ...
        saveFavoriteState();
        renderFavorites();
        showNotification(t("已添加到收藏列表"), "success");
    }
}
...
async function playFavoriteSong(index) {
    ...
    try {
        ...
        showNotification(t("播放收藏歌曲失败"), "error");
    }
}
...
function addAllFavoritesToPlaylist() {
    const favorites = ensureFavoriteSongsArray();
    if (favorites.length === 0) {
        showNotification(t("收藏列表为空"), "warning");
        return;
    }
    ...
    if (added > 0) {
        renderPlaylist();
        const duplicateHint = duplicates > 0 ? `，${duplicates} ${t("首已存在")}` : "";
        showNotification(`${t("已添加")} ${added} ${t("首收藏歌曲到播放列表")}${duplicateHint}`, "success");
    } else {
        updatePlaylistActionStates();
        showNotification(t("收藏歌曲均已在播放列表中"), "warning");
    }
}
...
function clearFavorites() {
    ...
    updateFavoriteIcons();
    updatePlayModeUI();
    showNotification(t("收藏列表已清空"), "success");
    clearLyricsIfLibraryEmpty();
}
...
function exportFavorites() {
    const favorites = ensureFavoriteSongsArray();
    if (favorites.length === 0) {
        showNotification(t("收藏列表为空，无法导出"), "warning");
        return;
    }
    ...
    try {
        ...
        document.body.removeChild(anchor);
        URL.revokeObjectURL(url);
        showNotification(`${t("已导出")} ${favorites.length} ${t("首收藏歌曲")}`, "success");
    } catch (error) {
        console.error("导出收藏列表失败:", error);
        showNotification(t("导出收藏列表失败"), "error");
    }
}
...
function handleImportFavoritesChange(event) {
    ...
    reader.onload = () => {
        try {
            ...
            if (added > 0) {
                ...
                showNotification(t("文件中的歌曲已在收藏列表中"), "warning");
            }
        } catch (error) {
            console.error("导入收藏列表失败:", error);
            showNotification(t("导入收藏列表失败，请确认文件格式"), "error");
        } finally {
            if (input) {
                input.value = "";
            }
    ...
    reader.onerror = () => {
        ...
        showNotification(t("无法读取收藏列表文件"), "error");
        if (input) {
            input.value = "";
        }
    };
...
function clearPlaylist() {
    ...
    savePlayerState();
    showNotification(t("播放列表已清空"), "success");
    clearLyricsIfLibraryEmpty();
}
...
async function playSong(song, options = {}) {
    ...
    try {
        ...
        if (usedFallbackAudio) {
            ...
            showNotification(t('主音频加载失败，已切换到备用音源'), 'warning');
        }
...
async function exploreOnlineMusic() {
    ...
    try {
        ...
        if (!Array.isArray(results) || results.length === 0) {
            showNotification(t("探索雷达：未找到歌曲"), "error");
        ...
        if (appendedSongs.length === 0) {
            showNotification(t("探索雷达：本次未找到新的歌曲，当前列表已包含这些曲目"), "info");
        ...
        showNotification(`${t("探索雷达：新增")}${appendedSongs.length}${t("首")} ${randomGenre} ${t("歌曲")}`);
        ...
        showNotification(t("探索雷达获取失败，请稍后重试"), "error");
    } finally {
...
async function downloadSong(song, quality = "320") {
    try {
        ...
        if (audioData && audioData.url) {
            ...
            showNotification(t("下载已开始"), "success");
        } else {
